            ...
            <CardContent className="p-5 sm:p-6 space-y-3">
              <div className="flex items-start justify-between gap-4">
                ...
                <Badge variant="secondary" className="font-normal">
                  {codingAttempts?.length ? `${codingAttempts.length} attempts` : 'Ready'}
                </Badge>
              </div>
              <p className="mt-4 text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">
                Coding Lab & Mocks
              ...
              <p className="text-xs text-muted-foreground line-clamp-1">
                AI progressive coach, timed mocks, and Capgemini-style tests
              </p>
              <Progress value={Math.min(100, (codingAttempts?.length || 0) * 20)} className="h-1.5 mt-2" />
              <Button asChild variant="link" className="mt-2 h-auto p-0">
                <Link to="/coding-lab">
                  Enter Coding Lab <ArrowRight className="h-4 w-4 ml-1" />
                </Link>
              </Button>
            </CardContent>
          </Card>
