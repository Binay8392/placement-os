...
const moreNavItems = [
  { to: '/daily-plan', icon: CalendarDays, label: 'Planner' },
...
const allNavItems = [
  ...
  { to: '/daily-plan', icon: CalendarDays, label: 'Daily Planner' },
  { to: '/tasks', icon: ListChecks, label: 'Task Tracker' },
  { to: '/analytics', icon: BarChart3, label: 'Analytics' },
  { to: '/leetcode', icon: Terminal, label: 'LeetCode' },
        ...
        <div className="flex items-center justify-around px-2 py-2">
          {navItems.map(({ to, icon: Icon, label }) => {
            ...
            return (
              <RouterNavLink key={to} to={to}
                className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200",
                  isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
                <div className={cn("p-1.5 rounded-xl transition-all duration-200", isActive && "gradient-primary shadow-glow")}>
                  <Icon className={cn("w-5 h-5 transition-all", isActive && "text-primary-foreground")} />
                </div>
          ...
          <Drawer open={drawerOpen} onOpenChange={setDrawerOpen}>
            <DrawerTrigger asChild>
              <button className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all duration-200",
                isMoreActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
                <div className={cn("p-1.5 rounded-xl transition-all duration-200", isMoreActive && "gradient-primary shadow-glow")}>
                  <MoreHorizontal className={cn("w-5 h-5 transition-all", isMoreActive && "text-primary-foreground")} />
                </div>
                <span className="text-[10px] font-medium">More</span>
              </button>
            </DrawerTrigger>
            <DrawerContent>
              <DrawerHeader><DrawerTitle>More Features</DrawerTitle></DrawerHeader>
              <div className="p-4 pb-4 grid grid-cols-3 gap-4">
                {moreNavItems.map(({ to, icon: Icon, label }) => {
                  const isActive = location.pathname === to;
                  return (
                    <RouterNavLink key={to} to={to} onClick={() => setDrawerOpen(false)}
                      className={cn("flex flex-col items-center gap-2 p-4 rounded-xl transition-all duration-200",
...
export function DesktopSidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen fixed left-0 top-0 border-r border-border bg-sidebar p-4">
      <div className="mb-8"><Logo size="md" showTagline /></div>
        ...
        {allNavItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <RouterNavLink key={to} to={to}
              ...
              <Icon className={cn("w-5 h-5 transition-all", isActive && "text-primary")} />
              <span className="text-sm font-medium">{label}</span>
              {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full gradient-primary" />}
            </RouterNavLink>
          );
        })}
      </nav>
      <div className="pt-4 border-t border-border space-y-3">
        ...
        <p className="text-xs text-muted-foreground text-center">Stay focused. Stay consistent.</p>
      </div>
    </aside>
  );
}
